...
@section('title', 'Thêm thuộc tính lọc')
@section('page-title', 'Thêm thuộc tính lọc')
...
@section('content')
    <form id="admin-save-form" action="{{ route('admin.product-attributes.store') }}" method="POST">
        @csrf
        <div class="row g-4">
            <div class="col-lg-8">
                <x-card type="primary" :outline="true" title="Thông tin nhóm lọc" :collapsible="true" class="mb-4">
                    <div class="row g-3">
                        <div class="col-md-7"><x-input id="product_attribute_name" name="name" label="Tên thuộc tính" :value="$attribute->name" placeholder="Ví dụ: Loại da, Kiểu mi hoặc Chất liệu" required /></div>
        ...
        <div class="d-flex flex-wrap justify-content-end gap-2 mt-4 mb-5"><a class="btn btn-default" href="{{ route('admin.product-attributes.index') }}"><i class="bi bi-arrow-left me-1"></i>Quay lại</a><button class="btn btn-primary" type="submit"><i class="bi bi-plus-lg me-1"></i>Tạo thuộc tính lọc</button></div>
    </form>
@endsection
