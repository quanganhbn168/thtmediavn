    ...
    <form id="admin-save-form" action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                ...
                <x-card type="secondary" :outline="true" title="Video cảm nhận" :collapsible="true"><x-video-upload name="video" label="Video khách hàng" :existing-url="$testimonial->getFirstMediaUrl('testimonial_video') ?: null" placeholder="Tải video cảm nhận (không bắt buộc)" /><p class="text-muted small mb-0">Video sẽ hiển thị trong thẻ cảm nhận trên trang chủ.</p></x-card>
            </div>
        </div>
