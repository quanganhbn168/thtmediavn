...
@section('content')
    <form id="admin-save-form" action="{{ route('admin.brands.update', $brand) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row g-4">
            <div class="col-lg-8">
                ...
                <x-card type="secondary" :outline="true" title="Logo thương hiệu" :collapsible="true">
                    <x-image-upload
                        ...
                        placeholder="Kéo thả logo mới vào đây hoặc click để chọn file"
                        :max-files="1"
            ...
            <div class="col-lg-4">
                <x-card type="info" :outline="true" title="Cấu hình hiển thị" :collapsible="true" class="mb-4">
                    ...
                    <div class="border-top pt-3">
                        ...
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input cursor-pointer" type="checkbox" role="switch" name="is_active" id="brand_is_active" value="1" @checked((bool) old('is_active', $brand->is_active))>
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input cursor-pointer" type="checkbox" role="switch" name="is_featured" id="brand_is_featured" value="1" @checked((bool) old('is_featured', $brand->is_featured))>
                ...
                <x-card type="secondary" :outline="true" title="Liên kết nhanh" :collapsible="true">
                    <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                        <span class="text-muted">Sản phẩm đang gắn</span>
                        <span class="badge text-bg-light">{{ number_format($brand->products_count ?? 0) }}</span>
                    </div>

                    <div class="d-grid gap-2">
                        <a class="btn btn-outline-primary" href="{{ route('catalog', ['brand' => $brand->slug]) }}" target="_blank" rel="noopener">
                            <i class="bi bi-box-arrow-up-right me-1"></i>Xem trên website
                        </a>
                        @if(filled($brand->website))
                            <a class="btn btn-outline-secondary" href="{{ $brand->website }}" target="_blank" rel="noopener noreferrer">
                                <i class="bi bi-globe2 me-1"></i>Mở website chính thức
                            </a>
                        @endif
                    </div>
                </x-card>
