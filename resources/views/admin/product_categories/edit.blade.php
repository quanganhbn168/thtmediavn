...
@php
    $imageUrl = filled($category->image)
        ? (Str::startsWith((string) $category->image, ['http://', 'https://'])
            ? $category->image
            : asset(ltrim((string) $category->image, '/')))
        : null;
@endphp

@section('title', 'Sửa danh mục sản phẩm')
@section('page-title', 'Sửa danh mục sản phẩm')
...
@section('content')
    <form id="admin-save-form" action="{{ route('admin.product-categories.update', $category) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-8">
                ...
                <x-card type="secondary" :outline="true" title="Ảnh đại diện" :collapsible="true">
                    <x-image-upload
                        ...
                        placeholder="Kéo thả ảnh mới vào đây hoặc click để chọn file"
                        :max-files="1"
                    ...
                    <div class="border-top pt-3">
                        ...
                        <div class="form-check form-switch">
                            <input
                                ...
                                value="1"
                                @checked((bool) old('is_home', $category->is_home))
                            >
