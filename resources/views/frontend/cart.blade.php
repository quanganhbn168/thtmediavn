...
<section class="section-space">
    <div class="container">
        ...
        @else
            @php
                $freeShipRemain = max(0, 1000000 - $summary['subtotal']);
            @endphp
            <div data-cart-content>
            <div class="mb-4"><div class="d-flex justify-content-between gap-3 small mb-2"><span data-shipping-message>{{ $freeShipRemain > 0 ? 'Mua thêm '.number_format($freeShipRemain,0,',','.').'₫ để được miễn phí vận chuyển' : 'Đơn hàng đã được miễn phí vận chuyển' }}</span><span data-shipping-percent>{{ min(100, round($summary['subtotal'] / 1000000 * 100)) }}%</span></div><div class="shipping-progress"><div class="shipping-progress-bar" data-shipping-progress style="width: {{ min(100, $summary['subtotal'] / 1000000 * 100) }}%"></div></div></div>
            <div class="row g-4 align-items-start">
                <div class="col-lg-8">
                    ...
                    @foreach($cart->items as $item)
                        ...
                        <tr data-cart-row data-unit-price="{{ $itemUnavailable ? 0 : $item->unit_price }}" class="{{ $itemUnavailable ? 'opacity-75' : '' }}">
                            <td class="p-3"><div class="d-flex gap-3 align-items-center"><img class="cart-product-img" src="{{ $item->variant?->image ?: $item->product->image_url }}" alt="{{ $item->product->name }}" width="600" height="600"><div>@if($itemUnavailable)<span class="fw-bold d-block">{{ $item->product->name }}</span><span class="badge text-bg-warning">Không khả dụng</span>@else<a class="fw-bold d-block" href="{{ route('content.show', ['domain' => 'san-pham', 'slug' => $item->product->slug]) }}">{{ $item->product->name }}</a>@endif @if($item->variant)<div class="small text-muted mt-1">{{ $item->variant->name }}</div>@endif<form action="{{ route('cart.destroy',$item) }}" method="post" class="mt-1" data-cart-remove-form>@csrf @method('DELETE')<button class="btn btn-link text-critical p-0 small">Xóa</button></form></div></div></td>
                            ...
                            <td class="p-3 text-lg-end"><strong data-cart-line-total>{{ number_format($item->unit_price*$item->quantity,0,',','.') }}₫</strong></td>
                        </tr>
                    @endforeach
                    </tbody></table></div>
                ...
                <aside class="col-lg-4"><div class="cart-summary"><h2 class="h5 fw-bold mb-3">Tóm tắt đơn hàng</h2>
                    ...
                    @if($summary['unavailableItems']->isEmpty())
                        <a class="btn btn-primary w-100 py-3" href="{{ route('checkout') }}"><i class="bi bi-shield-lock me-2"></i>Tiến hành thanh toán</a>
                    @else
                        <button class="btn btn-secondary w-100 py-3" type="button" disabled>Gỡ sản phẩm không khả dụng để tiếp tục</button>
                    @endif
