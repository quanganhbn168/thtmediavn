                ...
                <aside class="col-lg-5 checkout-order-column">
                    <div class="cart-summary sticky-lg-top checkout-summary">
                        ...
                        @foreach($cart->items as $item)
                            <div class="checkout-product-row"><img class="cart-product-img" src="{{ $item->variant?->image ?: $item->product->image_url }}" alt="{{ $item->product->name }}" width="80" height="80"><div class="small flex-grow-1"><strong>{{ $item->product->name }}</strong>@if($item->variant)<div class="text-primary mt-1">{{ $item->variant->values->map(fn ($value) => ($value->option?->name ? $value->option->name.': ' : '').$value->value)->join(' · ') ?: $item->variant->name }}</div>@endif<div class="text-muted">Số lượng: {{ $item->quantity }}</div></div><strong class="small">{{ number_format($item->unit_price * $item->quantity, 0, ',', '.') }}₫</strong></div>
                        @endforeach
